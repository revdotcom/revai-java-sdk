...
public class ErrorInterceptorTest {
    ...
    public void setup() throws IOException {
        ...
        sampleRequest = new Request.Builder()
                .url("https://api.rev.ai/revspeech/v1/")
                .build();
        when(mockChain.request()).thenReturn(sampleRequest);
        sampleResponse = new Response.Builder()
                .code(200)
    ...
    public void InvalidParameterExceptionTest() throws IOException {
        when(mockChain.proceed(any(Request.class))).thenReturn(sampleResponse.newBuilder().code(400).build());
        try{
    ...
    public void AuthorizationExceptionTest() throws IOException {
        when(mockChain.proceed(any(Request.class))).thenReturn(sampleResponse.newBuilder().code(401).build());
        try{
            sut.intercept(mockChain);
            Assert.fail();
        } catch (Exception e){
    ...
    public void ResourceNotFoundExceptionTest() throws IOException {
        when(mockChain.proceed(any(Request.class))).thenReturn(sampleResponse.newBuilder().code(404).build());
        try{
            sut.intercept(mockChain);
            Assert.fail();
        } catch (Exception e){
